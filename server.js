...
const initServer = async () => {
  ...
  server.ext('onRequest', (request, h) => {
    ...
    return h
      .redirect(
        'https://www.buildnumber.dev' +
          (request.url.path || request.url.pathname + request.url.search)
      )
      .takeover()
      .code(301)
  })
  ...
  server.route({
    method: 'GET',
    path: '/{param*}',
    vhost: ['www.buildnumber.dev', server.info.host, '127.0.0.1', 'localhost'],
    handler: {
      directory: {
  ...
  const addRoute = (params) => {
    ...
    server.route({
      ...params,
      vhost: [server.info.host, '127.0.0.1', 'localhost'],
  ...
  addRoute({
    ...
    handler: async (request, h) => {
      ...
      if (similarAppsCount > 0) {
        throw Boom.conflict(
          'There is already an app with the same bundle identifier for this account'
        )
      }
      ...
      // Email templates
      const emailTextContent = nunjucks.render('./email-templates/welcome-email.txt', {
        bundleIdentifier,
        tokenValue,
      })
      const emailHtmlContent = nunjucks.render('./email-templates/welcome-email-inlined.html', {
        bundleIdentifier,
        tokenValue,
      })
